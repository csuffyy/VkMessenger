...
namespace VkData.Account.Categories
{
        ...
        IStickers<Message, StickerSize>
    {
        ...
        public string Get(IEnumerable<object> attachments, StickerSize size)
        {
            var stickers = attachments.OfType<Sticker>().ToList();
            return stickers.Count == 0 ? null : Get(size, stickers.First());
        }

        public string Get(Message message, StickerSize size)
        {
        ...
        public string Get(string url) => Account.Downloader.DownloadAsync(url, GetPath(url), PathOptions.Full);
        ...
        private string Get(StickerSize size, Sticker sticker)
            => sticker != null ? Get(_stickerMappings[size](sticker)) : null;

        private string GetPath(string url)
            => $@"{Path}\{GetSize(url, Account.AppSettings.PhotoExtension)}\{GetId(url)}{Account.AppSettings.PhotoExtension}";
    }
}